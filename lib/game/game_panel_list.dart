...
class _GamePanelListState extends State<GamePanelList>
  ...
  ExpansionPanel gameExpansionPanel(Game _game, int _index) {
    return ExpansionPanel(
        headerBuilder: (context, isOpen) => Hero(
            tag: 'game' + _index.toString(), child: GameHeader(game: _game)),
        body: GameBody(
          game: _game,
          index: _index,
          isDescription: false,
          onPressedPrimary: Common.currentScreen == Activity.MyGames &&
                  !Common.canBeInstalledOnThisPlatform(_game)
              ? null
